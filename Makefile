...
build/output.yaml: template.yaml build/layer.zip
	aws --profile gkk --region eu-central-1 cloudformation package --template $< --s3-bucket $(DEPLOYMENT_BUCKET) --output-template-file $@

deploy: build/output.yaml
	aws --profile gkk --region eu-central-1 cloudformation deploy --template $< --stack-name $(STACK_NAME)
